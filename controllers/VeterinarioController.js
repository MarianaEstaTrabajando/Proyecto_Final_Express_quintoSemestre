...
module.exports = {
    //Gatos métodos
     async listar(req, res) {
        try {
            ...
            res.render('trabajador/voluntarios', { usuarios });
        } catch (error) {
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE (u.nombre LIKE :nombre OR u.apellido1 LIKE :nombre) and ur.rol_id=5
                 `,
                {
                    type: sequelize.QueryTypes.SELECT,
   ...
   async listarId(req, res) {
        try {
            ...
            res.render("trabajador/voluntario_detalles", { usuario: usuarioObj });
    ...
    async listar(req, res) {
        try {
            ...
            res.render('trabajador/veterinarios', { usuarios });
        } catch (error) {
            res.status(500).send("Error al obtener usuarios: " + error.message);
        }
    },
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE (u.nombre LIKE :nombre OR u.apellido1 LIKE :nombre) and ur.rol_id=4`,
                {
                    type: sequelize.QueryTypes.SELECT,
                    replacements: { nombre: `%${nombre}%` }
                }
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE u.edad = :edad and ur.rol_id=4`,
                {
                    type: sequelize.QueryTypes.SELECT,
                    replacements: { edad }
                }
    ...
    async listarId(req, res) {
        ...
        console.log("telefonos:",telefonos);
            ...
            res.render("trabajador/veterinario_detalles", { usuario: usuarioObj });
  ...
  formulario(req, res) {
    ...
    res.render("trabajador/input_veterinario", {user});
  },
  ...
  async guardar(req, res) {
    try {
        ...
        const Veterinario = await Usuario.create({
            nombre,
            apellido1,
            apellido2,
            edad,
        ...
        res.redirect("/trabajador/listar_veterinarios");
    } catch (error) {
     
        res.status(500).send("Error al guardar veterinario: " + error.message);
    }
},
...
async modificarFormulario(req, res) {
  try {
    ...
    const usuarioRol = await UsuarioRol.findOne({ where: { usuario_id: id, rol_id: 4 } });
    if(!usuarioRol) return res.status(404).send("Relación Usuario-Rol no encontrada");
    if (!usuario) return res.status(404).send("Usuario no encontrado");
   
    res.render("trabajador/modificar_veterinario", { usuario,usuarioRol,user:req.session.user });
  } catch (error) {
...
async modificar(req, res) {
  try {
    ...
    const usuarioRol = await UsuarioRol.findOne({
      where: { usuario_id: id, rol_id: 4 } 
    });
    if (usuarioRol) {
      await usuarioRol.update({ salario: parseFloat(salario) });
    }
    res.json({ mensaje: "Veterinario modificado correctamente" });
  } catch (error) {
    res.status(500).send("Error al modificar veterinario: " + error.message);
  }
},
...
async subirDireccion(req, res) {
    try{
        ...
        if(usuarioRol.rol_id !=4){
            return res.status(403).send("El usuario no es un veterinario");
        }

        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
         
            return res.status(403).send("No tiene permiso para agregar dirección a este veterinario");
        }
...
async borrarDireccion(req, res) {
    try{
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            ...
            return res.status(403).send("No tiene permiso para borrar dirección a este veterinario");
        }
...
async subirTelefono(req, res) {
    try{
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            ...
            return res.status(403).send("No tiene permiso para agregar teléfono a este veterinario");
        }
...
async borrarTelefono(req, res) {
    try{    
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            ...
            return res.status(403).send("No tiene permiso para borrar número a este veterinario");
        }
    ...
    async aprobarUsuario(req, res) {
        try {
            ...
            if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
               return res.status(403).send("No tiene permiso para aprobar a este veterinario");
            }
    ...
    async rechazarUsuario(req, res) {
        try {
            ...
            if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
                ...
                return res.status(403).send("No tiene permiso para rechazar a este veterinario");
            }
    ...
    async borrarFoto(req, res) {
        try {
            ...
            res.json({ mensaje: "Foto eliminada", fotoEliminada: foto.id });
        } catch (error) {
