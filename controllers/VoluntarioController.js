...
module.exports = {
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE u.edad = :edad and ur.rol_id=5`,
                {
                    type: sequelize.QueryTypes.SELECT,
                    replacements: { edad }
                }
  ...
  formulario(req, res) {
    ...
    res.render("trabajador/input_voluntario", {user});
  },
  ...
  async guardar(req, res) {
    try {
        ...
        const Voluntario = await Usuario.create({
            nombre,
            apellido1,
            apellido2,
            edad,
        ...
        res.redirect("/trabajador/listar_voluntarios");
    } catch (error) {
        
        res.status(500).send("Error al guardar voluntario: " + error.message);
    }
},
...
async modificarFormulario(req, res) {
  try {
    ...
    const usuarioRol = await UsuarioRol.findOne({ where: { usuario_id: id, rol_id: 5 } });
    if(!usuarioRol) return res.status(404).send("Relación Usuario-Rol no encontrada");
    if (!usuario) return res.status(404).send("Usuario no encontrado");
 
    res.render("trabajador/modificar_voluntario", { usuario,usuarioRol,user:req.session.user });
  } catch (error) {
...
async modificar(req, res) {
  try {
    ...
    const usuarioRol = await UsuarioRol.findOne({
      where: { usuario_id: id, rol_id: 5 }
    });
    if (usuarioRol) {
      await usuarioRol.update({ salario: parseFloat(salario) });
    }
   
    res.json({ mensaje: "Voluntario modificado correctamente" });
  } catch (error) {
   
    res.status(500).send("Error al modificar voluntario: " + error.message);
  }
},
...
async subirDireccion(req, res) {
    try{
        ...
        if(usuarioRol.rol_id !=5){
            return res.status(403).send("El usuario no es un voluntario");
        }

        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
          
            return res.status(403).send("No tiene permiso para agregar dirección a este voluntario");
        }
...
async borrarDireccion(req, res) {
    try{
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            ...
            return res.status(403).send("No tiene permiso para borrar dirección a este voluntario");
        }
...
async subirTelefono(req, res) {
    try{
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            ...
            return res.status(403).send("No tiene permiso para agregar teléfono a este voluntario");
        }
...
async borrarTelefono(req, res) {
    try{    
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            ...
            return res.status(403).send("No tiene permiso para borrar número a este voluntario");
        }
    ...
    async aprobarUsuario(req, res) {
        try {
            ...
            if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
                ...
                return res.status(403).send("No tiene permiso para aprobar a este voluntario");
            }
    ...
    async rechazarUsuario(req, res) {
        try {
            ...
            if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
                ...
                return res.status(403).send("No tiene permiso para rechazar a este voluntario");
            }
