...
module.exports = {
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id, ur.rol_id, ur.estado_usuario
                 ...
                 where ur.rol_id= 6 and ur.estado_usuario='aprobado'`,
            ...
            res.render('trabajador/adoptantes', { usuarios });
        } catch (error) {
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE (u.nombre LIKE :nombre OR u.apellido1 LIKE :nombre) and ur.rol_id=6 and ur.estado_usuario='aprobado'`,
                {
                    type: sequelize.QueryTypes.SELECT,
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE u.edad = :edad and ur.rol_id=6 and ur.estado_usuario='aprobado'`,
                {
                    type: sequelize.QueryTypes.SELECT,
                    replacements: { edad }
                }
    ...
    async listarId(req, res) {
        try {
         ...
         const pertenece = !user.refugio_id || usuarioRaw.refugio_id == user.refugio_id;
         if(!pertenece) return;
            ...
            res.render("trabajador/adoptante_detalles", { usuario: usuarioObj });
        ...
        async listarRechazados(req, res) {
        try {
            const user = req.session.user;
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id, ur.rol_id, ur.estado_usuario
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 where ur.rol_id= 6 and ur.estado_usuario='rechazado'`,
                { type: sequelize.QueryTypes.SELECT }
            );
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id, ur.rol_id, ur.estado_usuario
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 where ur.rol_id= 6 and ur.estado_usuario='proceso'`,
                { type: sequelize.QueryTypes.SELECT }
            );
  ...
  formulario(req, res) {
    ...
    res.render("trabajador/input_adoptante", {user});
  },
...
async modificarFormulario(req, res) {
  try {
    ...
    const usuarioRol = await UsuarioRol.findOne({ where: { usuario_id: id, rol_id: 6 } });
    if(!usuarioRol) return res.status(404).send("Relación Usuario-Rol no encontrada");
    if (!usuario) return res.status(404).send("Usuario no encontrado");

    res.render("trabajador/modificar_adoptante", { usuario,usuarioRol,user:req.session.user });
  } catch (error) {
...
async modificar(req, res) {
  try {
    const  id  = parseInt(req.params.id);
    let { nombre, apellido1, apellido2, edad, usuario, password } = req.body;
    ...
    const usuarioRol = await UsuarioRol.findOne({
      where: { usuario_id: id, rol_id: 6 }
    });
    if (usuarioRol) {
      await usuarioRol.update({ salario: parseFloat(salario) });
    }
    res.json({ mensaje: "Adoptante modificado correctamente" });
  } catch (error) {
    
    res.status(500).send("Error al modificar adoptante: " + error.message);
  }
},
