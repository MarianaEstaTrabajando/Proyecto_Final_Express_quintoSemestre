...
const Refugio=require('../Models/Entes/Modelo_Refugio')
const fs = require("fs");
...
module.exports = {
    ...
    async listar(req, res) {
        try {
            ...
            res.render('admin/admins_refugio', { usuarios }); 
        } catch (error) {
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE (u.nombre LIKE :nombre OR u.apellido1 LIKE :nombre) and ur.rol_id=2`, 
                {
                    type: sequelize.QueryTypes.SELECT,
        ...
        try {
            ...
            const usuariosRaw = await sequelize.query(
                `SELECT u.*, m.url, ur.refugio_id
                 ...
                 JOIN UsuarioRol ur ON u.id = ur.usuario_id
                 LEFT JOIN vw_UsuariosMultimedia m ON u.id = m.usuario_id AND m.tipo_archivo = 'foto'
                 WHERE u.edad = :edad and ur.rol_id=2`, 
                {
                    type: sequelize.QueryTypes.SELECT,
                    replacements: { edad }
                }
    ...
    async listarId(req, res) {
        try {
            ...
            const usuarioObj = {
                ...
                refugio_id:usuarioRaw[0].refugio_id,
                fotos: fotos,
        ...
        console.log("telefonos:",telefonos);
            ...
            res.render("admin/admin_refugio_detalles", { usuario: usuarioObj }); 
        } catch (error) {
  ...
  formulario(req, res) {
    ...
    res.render("admin/input_admin_refugio", {user}); 
  },
  ...
  async guardar(req, res) {
    try {
        ...
        const refugioExiste = await Refugio.findByPk(refugioId);

        if (!refugioExiste) {
        return res.json({ mensaje: "El refugio no existe" });
        }
        ...
        const AdminRefugio = await Usuario.create({ 
            nombre,
            apellido1,
            apellido2,
            edad,
        ...
        res.redirect("/admin/listar_admins_refugio"); 
    } catch (error) {
        res.status(500).send("Error al guardar admin_refugio: " + error.message); 
    }
},
...
async modificarFormulario(req, res) {
  try {
    ...
    const usuarioRol = await UsuarioRol.findOne({ where: { usuario_id: id, rol_id: 2 } }); 
    if(!usuarioRol) return res.status(404).send("Relación Usuario-Rol no encontrada");
    if (!usuario) return res.status(404).send("Usuario no encontrado");
    res.render("admin/modificar_admin_refugio", { usuario,usuarioRol,user:req.session.user });
  } catch (error) {
...
async modificar(req, res) {
  try {
    ...
    const usuarioRol = await UsuarioRol.findOne({
      where: { usuario_id: id, rol_id: 2 } 
    });
    if (usuarioRol) {
      await usuarioRol.update({ salario: parseFloat(salario) });
    }
    res.json({ mensaje: "Admin de Refugio modificado correctamente" }); 
  } catch (error) {
    res.status(500).send("Error al modificar admin_refugio: " + error.message); 
  }
},
...
async subirDireccion(req, res) {
    try{
        ...
        if(usuarioRol.rol_id !=2){ 
            return res.status(403).send("El usuario no es un admin_refugio"); 
        }

        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            return res.status(403).send("No tiene permiso para agregar dirección a este admin_refugio"); 
        }
...
async borrarDireccion(req, res) {
    try{
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            return res.status(403).send("No tiene permiso para borrar dirección a este admin_refugio"); 
        }
...
async subirTelefono(req, res) {
    try{
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            return res.status(403).send("No tiene permiso para agregar teléfono a este admin_refugio"); 
        }
...
async borrarTelefono(req, res) {
    try{    
        ...
        if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
            return res.status(403).send("No tiene permiso para borrar número a este admin_refugio"); 
        }
    ...
    async aprobarUsuario(req, res) {
        try {
            ...
            if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
                return res.status(403).send("No tiene permiso para aprobar a este admin_refugio"); 
            }
    ...
    async rechazarUsuario(req, res) {
        try {
            ...
            if(user.refugio_id!== usuarioRol.refugio_id && user.refugio_id){
                return res.status(403).send("No tiene permiso para rechazar a este admin_refugio"); 
            }
